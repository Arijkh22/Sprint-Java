 ...
 * @author Ammar
 */
public class FinancesForm extends SideMenuBaseForm{
    
    public FinancesForm(Resources res){
            super(BoxLayout.y());
        ...
        Container titleCmp = BoxLayout.encloseY(
                        ...
                        BorderLayout.centerAbsolute(
                                BoxLayout.encloseY(
                                    new Label(FinanceServices.currentFinance.getTaxe()+"-"+FinanceServices.currentFinance.getTaxe(), "Title"),
                                    new Label(FinanceServices.currentFinance.getEtat(), "SubTitle")
                                )
                   ...
                   // Map<String, Object> obj = new HashMap<>();
                    ...
                    for (Map<String, Object> obj : list) {
                       ...
                       edit= edit.fill(1, 1);
                        ...
                        System.out.println(finance.getPrix());
                        finance.setEtat(obj.get("etat").toString());
                        finance.setDate(obj.get("date").toString());
                        finance.setId_facture( ((int)(double) obj.get("ide")));
                           Label l = new Label(finance.getId()+finance.getEtat());
                        box.addComponent(0,l);
                            box.addComponent(1,  editButtonBottom(edit, " "+ "", 0x5ae29d, true,finance,(int)(double) obj.get("id"),res));
    ...
    private MultiButton addButtonBottom(Image arrowDown, String text, int color, boolean first, int id,Resources rs){
        ...
        ConnectionRequest con = new ConnectionRequest();
        con.setUrl("http://127.0.0.1:8000/deletefinancejson?id="+id+"");
         con.addResponseListener(new ActionListener<NetworkEvent>() {
                           @Override
                           public void actionPerformed(NetworkEvent evt) {
                                
                           new FinancesForm(rs).show();
                           }
...
delete= delete.fill(80, 80);
    ...
    private Image createCircleLine(int color, int height, boolean first) {
       ...
       private MultiButton editButtonBottom(Image arrowDown, String text, int color, boolean first, Finance finance,int id,Resources rs){
        ...
        con.setUrl("http://127.0.0.1:8000/afficherfinancejSON");
         con.addResponseListener(new ActionListener<NetworkEvent>() {
                           @Override
                           public void actionPerformed(NetworkEvent evt) {
                               FinanceServices.currentFinance =finance;
                                System.out.println(finance.getId());
                           new ModifierFinanceForm(rs,finance,id).show();
                           }
