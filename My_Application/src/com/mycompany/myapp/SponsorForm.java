...
public class SponsorForm extends SideMenuBaseForm{
    ...
    public SponsorForm(Resources res){
        ...
        Container titleCmp = BoxLayout.encloseY(
                        ...
                        BorderLayout.centerAbsolute(
                                BoxLayout.encloseY(
                                    new Label(SponsorService.currentsponsor.getNom()+"-"+SponsorService.currentsponsor.getNom(), "Title"),
                                    new Label(SponsorService.currentsponsor.getNom(), "SubTitle")
                                )
                      ...
                      Image delete = res.getImage("delete.png");
                       ...
                       edit= edit.fill(1, 1);
                        ...
                        sponsor.setNum_tel((((int)(double) obj.get("numTel"))));
                           Label l = new Label(sponsor.getId()+sponsor.getAdresse());
                        box.addComponent(0,l);
                            box.addComponent(1,  editButtonBottom(edit, " "+ "", 0x5ae29d, true,sponsor,res));
                            box.addComponent(2,  addButtonBottom(edit, " "+ "", 0x5ae29d, true,sponsor.getId(),res));
    ...
    private MultiButton addButtonBottom(Image arrowDown, String text, int color, boolean first, int id,Resources rs){
        ...
        con.setUrl("http://127.0.0.1:8000/deleteSponsorjson?id="+id+"");
         con.addResponseListener(new ActionListener<NetworkEvent>() {
                           @Override
                           public void actionPerformed(NetworkEvent evt) {
                                
                           new SponsorForm(rs).show();
                           }
...
delete= delete.fill(80, 80);
    ...
    private Image createCircleLine(int color, int height, boolean first) {
       ...
       private MultiButton editButtonBottom(Image arrowDown, String text, int color, boolean first, Sponsor sponsor,Resources rs){
        ...
        con.setUrl("http://127.0.0.1:8000/afficherSponsorjSON");
         con.addResponseListener(new ActionListener<NetworkEvent>() {
                           @Override
                           public void actionPerformed(NetworkEvent evt) {
                               SponsorService.currentsponsor =sponsor;
                                System.out.println(sponsor.getId());
                           new ModifierSponsorForm(rs,sponsor).show();
                           }
